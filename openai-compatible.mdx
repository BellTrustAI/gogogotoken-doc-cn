...
超时重试可带 `Idempotency-Key`（与主路径相同，见 [视频生成 · 幂等提交](/video-generations#idempotency)）；响应丢失可用 [`GET /v1/video/generations?request_id=`](/video-tasks#lookup-by-request-id) 找回。
...
<ParamField body="model" type="string" required default="doubao-seedance-2-0-fast">
  视频模型 ID，如 `doubao-seedance-2-0-fast`、`doubao-seedance-2-0`、`doubao-seedance-2-5`。
</ParamField>
