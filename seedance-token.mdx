...
<Note>
  当 `data.status` 为 `SUCCESS` 时，读取 `data.data.usage.completion_tokens` 作为结算依据。`doubao-seedance-2-0-fast` 与 `doubao-seedance-2-0-mini` 不支持 `1080p`，传 `1080x1920` 会被服务端拒绝。
</Note>
...
<ParamField body="size" type="string" default="1080x1920">
  分辨率/像素尺寸，如 `1080x1920`、`720x1280`、`480x854`。
</ParamField>
