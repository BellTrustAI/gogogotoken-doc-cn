...
`doubao-seedance-2-0` 与 `doubao-seedance-2-0-fast` 走 **ARK Token 计费**。提交与查询均使用 SD2 主路径 `/v1/video/generations`；任务完成后按 `data.data.usage.completion_tokens` 差额结算。
...
<Note>
  当 `data.status` 为 `SUCCESS` 时，读取 `data.data.usage.completion_tokens` 作为结算依据。`doubao-seedance-2-0-fast` 不支持 `1080p`，传 `1080x1920` 会被服务端拒绝。
</Note>
